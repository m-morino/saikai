...
> ## Claude Code のための、自分の機械の上の管制室。
> **自分の機械・自分のターミナル・自分の履歴で。** 複数セッションを並べたまま動かし、
> リポジトリや worktree をまたいで**どれが応答待ちか**をひと目で確認し、**Claude が
> すでに削除したセッション**もそのまま探せます。左に検索できる一覧、右に選んだ
> セッションがそのまま動きます。

![動いている saikai — 左の一覧で応答待ちが分かり、選ぶと右でライブに動き、複数を行き来し、肥大したセッションをチェックポイントで作り直す](docs/assets/saikai-demo-ja.gif)

**Claude の標準機能と併用できます。** 現在の `claude --resume` には検索・プレビュー・
全プロジェクト表示（`Ctrl+A`）があり、`claude agents` では実行中のエージェントを管理できます。
saikai は独自のお気に入り、会話本文の検索、保存したペインの復元、埋め込みのライブペインを
提供します。作業中・入力待ち・待機中のライブ状態は saikai 内のペインで取得し、
外部で動くセッションはレジストリと履歴から推定します。
公式の[セッション管理](https://code.claude.com/docs/en/sessions)と
[agent view](https://code.claude.com/docs/en/agent-view)も参照してください。
さらに、Claude Code が 30 日で削除したセッション（`cleanupPeriodDays`）も一覧に残します
（下の「Claude が消したセッションも、ここには残る」参照）。

## こんな人に向いています

複数のリポジトリや worktree で Claude Code を走らせていると、見失いがちです——どのセッション
だったか、どれが自分の応答待ちで止まっているか、あの作りかけの変更はどこへ行ったか。
saikai は会話を探し、よく使う作業セットを復元するためのターミナル画面です。
たとえばこんなとき:

- **どれが応答待ちか**を知りたい——動いている全セッションの状態を一目で見て、次に対応
  すべきものへキー一つで移りたい
- 複数の `claude` を同時に動かし、ターミナルのタブを増やさずに切り替えたい
- タイトルだけでは思い出せない作業を、本文・プレビュー・差分といった**中身**から探したい
- よく戻るものをお気に入りにし、作業セットごと後で開き直したい

ターミナルアプリにしたのには理由があります。Claude Code がそもそも CLI 中心で新機能も
そこから早く来るし、デスクトップアプリより軽い。横断ビューを、普段のターミナルに
そのまま置きました。
...
![split-live のレイアウト：左にセッション一覧、右にライブで動く Claude、そして両者を行き来する Enter / Ctrl+] / F2 / F3 キー](docs/assets/saikai-split-live.svg)

- **どれが応答待ちか、一目で。** 一覧は状態でグループ分けされ、**入力待ち（Needs input）
  が先頭**に並びます。「今あなたの応答待ち」だけにシアン1色を使い（`?` 入力待ち · `!` 完了・
  要応答）、それ以外は落ち着いた dim。カーソルは起動時に最初の要応答へ移り、キー一つで
  次の要応答へ進めます。
- **複数の Claude を同時に動かし、キー一つで切替。** 選んだセッションを右ペインで実プロセス
  として動かしたまま、別のセッションへ行き来できます。ターミナルのタブを増やさずに済みます。
- **中身でセッションを探す。** リポジトリ・worktree を横断してタイトル・本文・ID で検索。
  プレビュー（最初/最後のメッセージが先頭）で見分け、差分を見て、開始フォルダで再開。
- **お気に入りと系譜で見分ける。** よく戻るものは `f` で。プロンプト再利用・コンテキスト
  リセットをまたいだ親子関係の推定もできます。
- **閉じても作業セットは戻せる。** `Shift+F4` で前回のペイン一式を開き直せます。常駐
  プロセスもデータベースも持たず、Claude の履歴ファイルを読むだけ（AI 要約は任意）。
- **Claude が消したセッションも、ここには残る。** Claude Code は transcript を 30 日で
  削除します（`cleanupPeriodDays`・起動時に無警告で実行）。saikai は一度見たセッションの
  記録を自分で保持し、見たことがないものは Claude が決して削除しない
  `~/.claude/history.jsonl`（プロンプト記録）から復元します。`-` **期限切れ**として並び、
  読む・検索する・お気に入りに入れるはそのまま、再開だけができません。作者の実機では
  可視セッションが 12 → 74 件（12月まで遡及）になりました。`[history] archive = false`
  で無効化できます。
...
セッション一覧で `Space` → `w` を押すと作業セットの管理画面が開きます。
現在動いているClaudeペインを名前付きで保存し、再起動後にその名前を選ぶと、
保存した会話とフォルダを確認して復元できます。前回ペインの明示的な取り込み、
名前変更、現在のペインによる内容更新、削除もできます。更新前には確認画面を表示し、
空のペイン一覧で既存セットを上書きすることはありません。

ペインを閉じたり、復元時に一部をスキップしたりしても、保存済みセットの項目は減りません。
存在しないフォルダ、別ホスト、未対応provider、重複項目はプレビューに表示します。
既存のペイン数・メモリ制限と、別ウィンドウで開いている会話の確認は維持します。
起動受付と起動完了は区別します。

保存先は `platformdirs.user_data_dir("saikai")/workspaces.json` です。
キャッシュや `Shift+F4` の「前回ペイン」自動保存とは独立しています。
明示操作時だけロックと原子的置換で保存し、同時編集は上書きせず再読み込みを案内します。
壊れた保存ファイルも上書きせずエラーを表示します。
SSH端末のセッションを復元する場合は、そのホストでsaikaiを起動してください。
初版はそのホストのClaudeペインが対象で、SSH接続やエージェント間の会話移行は行いません。
...
- **静か**(dim): `=` 待機中 · `@` 別で開いている · `$` シェル実行中 · `R` Remote Control · `+` アクティブ · `.` 最近 · `&` バックグラウンド agent · `-` **期限切れ**(Claude が transcript を削除済み・saikai が記録を保持)
- **タグ**(別列): `*` お気に入り · `x` 非表示

![状態でグループ分けされた一覧（要応答が先頭）。シアンの「あなた待ち」アクセント・★ お気に入り・ステータスバーのソート/グループ表示](docs/assets/saikai-browse.svg)
...
日常操作: `↑` `↓` 移動 · `Enter` 開く/再開 · `F2`/`F3` でペイン切り替え · `Shift+F1` で次の
...
検索トークン `:fav` `:hidden` `:open` `:active` `:recent` `:expired`、`Alt+←/→` で境界移動、マウス操作、
