...
リポジトリや worktree をまたいで**全部を一覧**し、複数を同時に動かしたまま、どれが
作業中・入力待ち・応答待ちかを**シアン1色**で示して、必要なものへすぐ切り替えられます。

## こんな人に向いています

複数のリポジトリや worktree で Claude Code を走らせていると、見失いがちです——どのセッション
だったか、どれが自分の応答待ちで止まっているか、あの作りかけの変更はどこへ行ったか。
`claude --resume` は今のフォルダしか知らず、検索もプレビューも横断ビューもありません。
saikai はその穴を埋めます。たとえばこんなとき:
