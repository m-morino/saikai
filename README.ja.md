...
> **すべての Claude Code セッションのミッションコントロール。** 全リポジトリの
> セッションをライブペインで開き、瞬時に切り替え、"自分待ち" でグルーピング。
> **そして、膨らんで鈍ったセッションのやっかいなリセットも、いまはワンキーで安全に。**
...
saikai は、すべての Claude Code セッションをキーボード操作で束ねるミッション
コントロールです。複数の `claude` をライブペインとして同時に動かして瞬時に
切り替えられ、状態（作業中 `~` / 入力待ち `?` / 完了して返信待ち `!`）で
グルーピング・ソートできるので、どれに人手が要るかひと目で分かります。リポジトリや
worktree を横断して過去のセッションを探し、その場でライブ再開もできます。読みにいくのは
Claude 自身の履歴ファイルだけ。デーモンもデータベースも要りません。
...
**膨らんだセッション問題。** 長く使ったセッションは気づかないうちにコンテキスト
ウィンドウが埋まり、応答がだんだん鈍ります。でも直すのは面倒で、リセットすると全部
説明し直しになるので、つい劣化したまま auto-compact 任せで粘ってしまう。saikai は
これを見えるようにして、対処までひと続きにします。各ライブペインには、トランス
クリプトから読んだ**実際の**コンテキスト使用量が出ます（`ctx 662K/1.0M (66%)`、
緑→黄→赤）。どのセッションが重いかひと目で分かります。あとは **`Shift+F11`** で
`/compact` をその場に打ち込んで普段使いの軽量化。**`Space` `c`**（Checkpoint）なら
安全なフルリセットで、saikai がセッションに引き継ぎを書かせ、貼り付け可能な
`NEW SESSION PROMPT` を見せて**あなたの承認を待ち**、**Enter** を押したときだけ
`/clear` して身軽な新セッションに作り直します（**`Shift+F6`** で親セッションへ
戻れます）。saikai のほうから勝手に `/clear` することはありません。コンテキスト
ウィンドウを握っているのは Claude で、saikai は使用量を見せて標準コマンドを代わりに
叩くだけ、という建て付けです。

#### `claude --resume` ではダメなのか

`claude --resume` の候補は今いる作業ディレクトリを起点に出るので、作業が複数の
リポジトリや worktree にまたがると目的のセッションを探しづらくなります。saikai は
全プロジェクトを横断して1つの画面にまとめ、選んだセッションを一覧の隣でライブ
ペインとして動かします。Claude Desktop のような一覧を、普段のターミナルの中でも
使いたくて作りました。
...
- **すべてのセッションを1画面に。** リポジトリや worktree を横断して履歴を検索し、
  各セッション開始時の cwd から再開できます。
- **ライブペイン、瞬時の切り替え。** 複数の `claude` を split-live タブで動かし、
  キー一つで行き来します。
- **状態が一目で、要対応へジャンプ。** `~` 作業中 · `?` 入力待ち · `!` 完了・要応答
  でグルーピング／ソートし、次に人手が要るペインへ直行します。
- **終了してもペインを失わない。** `Shift+F4` で同じ作業セットを開き直せます。
  デーモンもデータベースも不要で、Claude 自身のトランスクリプトを読むだけです（AI 要約は任意）。
- **プレビュー・差分・再利用・追跡。** トランスクリプトのプレビュー、変更差分、
  プロンプト再利用、推定した親子セッションの追跡。
- **★ どのセッションが肥大かを見て、ワンキーで安全にリセット。** 各ペインは
  トランスクリプトの**実トークン数**を表示（`ctx 662K/1.0M (66%)`、chars/4 推定では
  ありません）、緑／黄／赤で色分け。`Shift+F11` は `/compact` をその場注入。
  **Checkpoint**（`Space` `c`）はセッションに引き継ぎを書かせ、reseed するプロンプトを
  提示して承認を求め、Enter で `/clear` → lean に再開（処理中は行に `↻`）。`Shift+F6`
  で親へ戻れます。
- **スマホ／別ブラウザへミラーリング**（任意・トークン認証・既定 read-only）。詳細は
  下記「Web ミラー」節を参照。
...
2. **`Space` がメニュー。** 一覧で `Space`、続けて 1 文字。`Space` の後に手が
   止まると、系統別の全メニューがその場に出ます（which-key 方式）。
   暗記は要りません:
...
対話型 `claude` プロセスを一覧の隣のタブで動かします。依存として一緒に入るので、
これが既定です。軽い一覧専用ブラウザ（`Enter` で全画面再開）に切り替えたいときは
環境変数を設定します:
...
saikai はライブ UI をスマホや別のブラウザにミラーリングできます。稼働中の様子を
ちらっと見たり、離れた場所からセッションを操作したり。**既定は無効**で、
**トークン認証**つきです。
...
いま統合済みのプロバイダは Claude Code です。エージェント固有の起動・状態の
取り決め（contract）は `saikai_provider.py` に切り出してあり、Codex 用の取り決めも
抽象化の検証用に置いていますが、履歴の発見とライブ状態の連携が仕上がるまで Codex は
選べるようにしていません。Claude の履歴探索は `CLAUDE_CONFIG_DIR` にも対応します。

## 周辺ツールとの関係

saikai は、時間をまたいでセッションを見つけ、再開し、状況を把握することに
重点を置いています。[ccmanager](https://github.com/kbwo/ccmanager) は、動作中の
複数のエージェントをまとめて調整するためのツールです。同じ問題を取り合うのでは
なく、隣り合う用途です。

## プラットフォーム対応

**正式に対応と言えるのは Windows 10 / 11・Python ≥ 3.11 だけです。**
Linux・WSL2・macOS の実装も残していますが、各 OS の実 PTY 経路で継続的な実機検証が
揃うまでは experimental 扱い。それ以外のプラットフォームは未対応です。
...
- Linux / WSL2 / macOS のレビュアーを募集しています。お使いのターミナル名、
  ローカルか SSH/tmux 経由か、PTY 終了・キー入力・クリップボードの結果を issue で
  共有してください。
