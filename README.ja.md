...
> **そして、肥大して鈍ったセッションの面倒なリセットも、いまや安全にワンキーで。**
...
コントロールです。複数の `claude` をライブペインとして同時に動かし、瞬時に切り替え、
状態 — 作業中 `~` / 入力待ち `?` / 完了して返信待ち `!` — でグルーピング・ソート
できるので、どれに人手が要るか常に分かります。リポジトリや worktree を横断して
任意のセッションを探し、ライブ再開できます。Claude 自身の履歴ファイルを直接読む
だけで、デーモンもデータベースも不要です。
...
**肥大したセッション問題。** 長いセッションは静かに context window を埋め、応答が
鈍っていきます。でも直すのは面倒 — リセットすれば全部説明し直しなので、つい劣化した
auto-compact に乗り続けてしまう。saikai はこの問題を可視化し、対処を一手にします。
各ライブペインは、トランスクリプトから読んだ**実際の** context 使用量を表示します
— `ctx 662K/1.0M (66%)`、緑 → 黄 → 赤 — どのセッションが重いか一目で分かります。
あとは **`Shift+F11`** で `/compact` をその場注入して普段使いの軽量化、**`Space` `c`**
（Checkpoint）で安全なフルリセット: saikai がセッションに引き継ぎを書かせ、貼り付け
可能な `NEW SESSION PROMPT` を提示して**あなたの承認を待ち**、**Enter** を押したときだけ
`/clear` して新しい lean なセッションに reseed します — **`Shift+F6`** で親セッションへ
戻れます。saikai が勝手に `/clear` することはありません。（context window を持つのは
Claude。saikai は使用量を可視化し、標準コマンドを駆動するだけです。）

#### `claude --resume` ではダメなのか

`claude --resume` の候補は現在の作業ディレクトリを起点に表示されるため、作業が
複数のリポジトリや worktree に広がると目的のセッションを探しづらくなります。
saikai は全プロジェクトを横断する1つのビューを提供し、選んだセッションを一覧の
隣でライブペインとして動かします。Claude Desktop のような一覧画面を、普段使う
ターミナル内でも使いたくて作りました。
...
- **終了してもペインを失わない。** `Shift+F4` で同じ作業セットを開き直せます。
  デーモンもデータベースも不要 — Claude 自身のトランスクリプトを読むだけ（AI 要約は任意）。
