...
saikai は、複数のリポジトリや worktree にまたがる Claude Code のセッションを
1画面に集約する TUI です。覚えている断片で検索し、選んだセッションを一覧の隣で
ライブ再開でき、どれが作業中・入力待ち・完了して返信待ちかが一目でわかります。
Claude 自身の履歴ファイルを直接読むだけで、デーモンもデータベースも不要です。
...
- ライブ UI をスマホや別のブラウザに LAN 経由でミラーリング（任意・トークン認証・
  既定は read-only）。`Shift+F12` で操作を有効化すると、タップ／スクロール／画面上の
  キーバー／物理キーボード（端末同等）で、ブラウザから saikai も — ペイン内の
  claude も — 操作できます。
