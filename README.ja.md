...
Claude Code のセッションが増えすぎて、どの会話がどこにあるかわからなくなってきた人へ。
saikai は `~/.claude/projects` の履歴全体を検索可能な 1 つの表に集約し、
日付 / プロジェクト / トピックでグループ化・絞り込みして、`Enter` 一発で再開します。
...
`Space` の後に迷って手が止まると、系統別にまとまった全マップがその場に表示
されます（which-key 方式。`?` でも同じマップを確認可能）:
...
| `y` プロンプトをコピー (yank) | `t` ツリー · `c` クラスタ | `a` 次の要対応ペインへ |
| `d` 変更内容 (diff) | `l` 一覧の表示/非表示 | `x` タブを閉じる · `[` `]` タブ移動 |
| `r` 再読込 | `,` 設定 (Settings) | `Space` 一括起動用マーク |
...
- **分割幅の変更**: `Alt+←` / `Alt+→` で一覧/ペインの境界を動かせます
  （位置はドラッグ時と同様に保存されます）。
- **ドロップダウン**: フィルタバー（検索ボックス + Group / Sort / Status / Age）
  は既定で表示されています。`Tab` / `Shift+Tab` でドロップダウンに入り、`Enter`
  で開きます。`Esc` でバーを隠して行数を稼げます（選択は記憶されます）。`/` か
  文字入力で再表示。
