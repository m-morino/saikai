...
> ## 動いている Claude Code セッションぜんぶの、ライブ管制室。
...
応答待ちかを**シアン1色**で一目に示します。ファイルを開き直すのと、管制室に座るのの違い。
...
- **誰が自分を待っているか**を知りたい——動いている全セッションの状態を一目で見て、次に
  対応すべきものへキー一つで飛びたい
- 複数の `claude` を**ライブ**で並走させ、ターミナルのタブの山を作らずに即座に切り替えたい
- タイトルだけでは思い出せない作業を、本文・プレビュー・差分といった**中身**から探したい
- よく戻るものをお気に入りにし、作業セットごと後で戻したい

ターミナルアプリにしたのには理由があります。Claude Code がそもそも CLI 中心で新機能も
そこから早く来るし、デスクトップアプリより軽い。管制室のような横断ビューを、普段の
ターミナルにそのまま置きました。
...
- **誰があなたを待っているか、一目で。** 一覧は状態でグループ分けされ、**入力待ち（Needs
  input）が先頭**。「今あなたを待っている」だけにシアン1色を予約（`?` 入力待ち · `!` 完了・
  要応答）、それ以外は落ち着いた dim。カーソルは起動時に最初の要応答へ*落ち*、キー一つで
  次の要応答へ飛べます。
- **複数の Claude をライブで並走、キー一つで切替。** 選んだセッションを右ペインで実プロセス
  として動かし、行き来できます——ターミナルのタブの山ではなく、見張って操舵できる艦隊。
