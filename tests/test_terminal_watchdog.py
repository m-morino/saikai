...
def test_win_pid_index_distinguishes_empty_from_failed_snapshot():
    """CreateToolhelp failures are unknown; a completed empty walk is conclusive."""
    import ctypes
    ...
    try:
        for kernel in (
            ...
            assert saikai._win_pid_index() is None
        ctypes.windll = type("_Windll", (), {
            "kernel32": _Kernel32(_Fn(1), _Fn(0), _Fn(0))})()
        assert saikai._win_pid_index() == {}
        ctypes.windll = type("_Windll", (), {
            "kernel32": _Kernel32(_Fn(1), _Fn(0), _Fn(0), last_error=5)})()
        assert saikai._win_pid_index() is None
        ctypes.windll = type("_Windll", (), {
            "kernel32": _Kernel32(_Fn(1), _Fn(1), _Fn(0), last_error=5)})()
        assert saikai._win_pid_index() is None
        ctypes.windll = type("_Windll", (), {
            "kernel32": _Kernel32(_Fn(1), _Fn(1), _Fn(0))})()
        assert isinstance(saikai._win_pid_index(), dict)
    finally:
...
def test_destructive_pid_check_requires_exact_windows_creation_ticks():
    ...
    try:
        saikai.sys.platform = "win32"
        saikai._win_pid_index = lambda: {42: ("claude.exe", 7)}
